...
class MiniGridShieldHandler(ShieldHandler):
    def __init__(self, grid_to_prism_binary, grid_file, prism_path, formula, prism_config=None, shield_value=0.9, shield_comparison='absolute', nocleanup=False, prism_file=None) -> None:
        self.tmp_dir_name = f"/opt/notebooks/shielding_files_{datetime.datetime.now().strftime('%Y%m%dT%H%M%S')}_{next(tempfile._get_candidate_names())}"
        try:
            os.mkdir(self.tmp_dir_name)
            print(f"Directory created successfully: {self.tmp_dir_name}")
        except Exception as e:
            print(f"Failed to create directory: {e}")
        # if os.path.isdir(self.tmp_dir_name):
        #     print(f"Directory exists: {self.tmp_dir_name}")
        # else:
        #     print(f"Directory does not exist: {self.tmp_dir_name}")
    ...
    def __create_prism(self):
        ...
        if self.prism_config is None:
            ...
            print(F"{self.grid_to_prism_binary} -i {self.grid_file} -o {self.prism_path}")
        else:
            result = os.system(F"{self.grid_to_prism_binary} -i {self.grid_file} -o {self.prism_path} -c {self.prism_config}")
            print(F"{self.grid_to_prism_binary} -i {self.grid_file} -o {self.prism_path} -c {self.prism_config}")
...
def create_shield_overlay_image(env, shield):
    ...
    # img.show()
    img.save(f"./figures/shield_overlay_image_{datetime.datetime.now().strftime('%Y%m%dT%H%M%S')}.png")
    print("Image saved as shield_overlay_image.png")
